...
const fullCacheName = `${cacheName}${cacheVersionPrefix}${cacheVersion}`;
...
self.addEventListener('fetch', function(event) {
  // "use strict";
  try {
    consoleWrapper(`[Service Worker] Event: fetch ${event.request.url}`);
    event.respondWith(
...
self.addEventListener('message', (event) => {
  if (event.data.type === 'debug') {
    ...
    console.log(`[Service Worker] DEBUG: ${debugValue}`);
  } else if (event.data === 'info') {
    console.log('[Service Worker] Info:');
    console.log(`[Service Worker] Base domain: ${baseDomain}`);
    console.log(`[Service Worker] Cache name: ${cacheName}`);
    console.log(`[Service Worker] Cache version: ${cacheVersion}`);
    console.log(`[Service Worker] Full cache name: ${fullCacheName}`);
    console.log(`[Service Worker] Manifest: ${baseDomain}/sainternet.webmanifest`);
    console.log(`[Service Worker] Service worker: ${baseDomain}/serviceworker.min.js`);
