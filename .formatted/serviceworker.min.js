 ...
 *
 * Cache recreated on fetch events if missing.
 *
 * External CSS and font GETs fetched with no-cors and then cloned to the
 * cache if successful.
 *
      ...
      .then((response) => {
        ...
        return caches.keys().then((keys) => {
          if (!keys.includes(cacheName)) {
            logDebugMessage(`[Service Worker] Cache not found, creating new cache: ${cacheName}`);
            return caches.open(cacheName)
              .then((cache) => cache.addAll(allURLS));
          }
        })
        .then(() => {
          logDebugMessage(`[Service Worker] Fetching uncached resource: ${event.request.url}`);
          let fetchRequest = event.request.clone();
          if (fetchRequest.url.endsWith('.css') || fetchRequest.url.endsWith('.woff') || fetchRequest.url.endsWith('.woff2') || fetchRequest.url.endsWith('.ttf')) {
              logDebugMessage(`[Service Worker] Fetching external font or css: ${fetchRequest.url}`);
              fetchRequest = new Request(fetchRequest, {mode: 'no-cors'});
          }
          return fetch(fetchRequest).then((response) => {
              if (response.status !== 404 && fetchRequest.method === 'GET') {
                const responseToCache = response.clone();
                if (fetchRequest.url.endsWith('.css') || fetchRequest.url.endsWith('.woff') || fetchRequest.url.endsWith('.woff2') || fetchRequest.url.endsWith('.ttf')) {
                  caches.open(cacheName).then((cache) => {
                    logDebugMessage(`[Service Worker] Cacheing external font or css: ${fetchRequest.url}`);
                    cache.put(event.request, responseToCache);
                  });
                }
              }
              logDebugMessage(`[Service Worker] Fetch response status: ${response.status} for URL: ${event.request.url}`);
              return response;
          });
        });
      })
