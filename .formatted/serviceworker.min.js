...
const debugChannel = new BroadcastChannel('sainternet-debug');

let debugValue = false;

/**
 * Sets the value of debugValue when a message is received on debugChannel.
 *
 * @param {MessageEvent} event - The message event received on debugChannel.
 * @param {*} event.data.debugValue - The value to set debugValue to.
 * @return {void}
 */
debugChannel.onmessage = (event) => {
  logDebugMessage(`[Service Worker] Broadcast Channel API message event: ${debugChannel}`);
  debugValue = event.data.debugValue;
};
