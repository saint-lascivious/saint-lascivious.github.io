 ...
 */
function consoleMessage(message, method = 'log') {
  // "use strict";
  if (window.console) {
...
self.addEventListener('install', function (event) {
  // "use strict";
  consoleMessage('[Service Worker] Event: install');
  try {
    self.skipWaiting();
    event.waitUntil(
      caches.open(PWA_CACHE_NAME).then(function (cache) {
        consoleMessage('[Service Worker] Installing cache: ' + PWA_CACHE_NAME);
        return cache.addAll(ALL_URLS);
      })
    );
  } catch (error) {
    consoleMessage('[Service Worker] Error during install event', 'error');
  }
});

self.addEventListener('activate', function (event) {
  // "use strict";
  consoleMessage('[Service Worker] Event: activate');
  try {
    if (self.clients && clients.claim) {
      consoleMessage('[Service Worker] Claiming available clients');
      clients.claim();
    }
    event.waitUntil(
      caches.keys().then(function (keyList) {
        ...
        return Promise.all(keyList.map(function (key, i) {
          if (cacheWhitelist.indexOf(key) === -1) {
            consoleMessage('[Service Worker] Deleting cache: ' + keyList[i] );
    ...
    consoleMessage('[Service Worker] Error during activate event', 'error');
  }
});

self.addEventListener('fetch', function (event) {
  // "use strict";
  consoleMessage('[Service Worker] Event: fetch');
  try {
    consoleMessage('[Service Worker] Fetch request: ' + event.request.url);
    event.respondWith(
      caches.match(event.request).then(function (request) {
        if (request) {
          consoleMessage('[Service Worker] Responding from cache: ' + event.request.url);
          return request;
        } else {
          consoleMessage('[Service Worker] Resource not cached, fetching: ' + event.request.url);
          return fetch(event.request);
        }
      })
    );
  } catch (error) {
    consoleMessage('[Service Worker] Error during fetch event', 'error');
  }
});
