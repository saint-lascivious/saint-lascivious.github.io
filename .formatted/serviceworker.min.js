...
/**
 * Event listener for the fetch event with a network first cache policy,
 * triggered whenever the Service Worker intercepts a network request.
 *
 * Uses no-cors mode to allow for caching of external resources.
 *
 * @param {FetchEvent} event - The fetch event object.
 * @throws {Error} If an error occurs during the event listener.
 */
  ...
  try {
    ...
    event.respondWith(
      fetch(event.request, {mode: 'no-cors'}).then(function(response) {
        ...
        logDebugMessage(`[Service Worker] Responding with network data: ${event.request.url}`);
        return response;
      }).catch(function() {
        logDebugMessage(`[Service Worker] Fetch failed, trying cache: ${event.request.url}`);
        return caches.match(event.request).then(function(cachedResponse) {
          if (cachedResponse) {
            logDebugMessage(`[Service Worker] Cache hit, responding with cached data: ${event.request.url}`);
            return cachedResponse;
          } else {
            logDebugMessage(`[Service Worker] Cache miss, could not serve request: ${event.request.url}`);
          }
        });
      })
    );
