...
self.importScripts('/resources/javascript/debug.min.js');

const PWA_NAME = 'sainternet';
const PWA_VERSION = '_debug_v0.1';

const PWA_CACHE_NAME = PWA_NAME + PWA_VERSION;

const APP_URLS = [
  'https://sainternet.xyz/sainternet.min.js',
  'https://sainternet.xyz/serviceworker.min.js',
...
const GOOGLE_JS_URLS = [
  'https://www.googletagmanager.com/gtm.js?id=GTM-5DT44KL',
];
...
const HTML_URLS = [
  ...
  'https://sainternet.xyz/debug.html',
...
const JS_URLS = [
  'https://sainternet.xyz/resources/javascript/debug.min.js',
  ...
  'https://sainternet.xyz/resources/javascript/toggle_drawer.min.js',
];
  ...
  try {
    ...
    event.waitUntil(
      caches.open(PWA_CACHE_NAME).then(function(cache) {
        logDebugMessage('[Service Worker] Installing cache: ' + PWA_CACHE_NAME);
        return Promise.all(
          ALL_URLS.map(function(url) {
            return fetch(new Request(url, {mode: 'no-cors'}))
              .then(function(response) {
                if (response.ok) {
                  logDebugMessage('[Service Worker] Adding URL to cache:', url);
                  return cache.put(url, response);
                } else {
                  logDebugMessage('[Service Worker] Fetch failed for URL:', url);
                }
              })
              .catch(function(error) {
                logDebugMessage('[Service Worker] Fetch failed', 'error');
              });
          })
        );
      })
    ...
    logDebugMessage('[Service Worker] Error during install event', 'error');
  }
});
...
self.addEventListener('activate', function(event) {
  ...
  try {
    ...
    event.waitUntil(
      caches.keys().then(function(keyList) {
        const cacheWhitelist = keyList.filter(function(key) {
          return key.indexOf(PWA_NAME);
        });
        cacheWhitelist.push(PWA_CACHE_NAME);
        return Promise.all(keyList.map(function(key, i) {
          if (cacheWhitelist.indexOf(key) === -1) {
            logDebugMessage('[Service Worker] Deleting cache: ' + keyList[i] );
    ...
    logDebugMessage('[Service Worker] Error during activate event', 'error');
  }
});
...
self.addEventListener('fetch', function(event) {
  // "use strict";
  try {
    logDebugMessage('[Service Worker] Fetch request: ' + event.request.url);
    event.respondWith(
    ...
    logDebugMessage('[Service Worker] Error during fetch event', 'error');
  }
});

self.addEventListener('message', (event) => {
  // "use strict";
  logDebugMessage('[Service Worker] Event: message');
  try {
    if (event.data.type === 'getDebugFlag') {
      const debugFlag = localStorage.getItem('DEBUG') ?? 'false';
      event.source.postMessage({type: 'debugFlag', value: debugFlag});
    } else if (event.data.type === 'setDebugFlag') {
      const debugFlag = event.data.value;
      localStorage.setItem('DEBUG', debugFlag);
    }
  } catch (error) {
    logDebugMessage('[Service Worker] Error during message event', 'error');
  }
});
