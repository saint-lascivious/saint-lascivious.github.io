      ...
      caches.open(fullCacheName).then(function(cache) {
        ...
        return cache.keys().then(function(keys) {
          keys.forEach(function(request) {
            consoleWrapper(`[Service Worker] ${request.url}`);
          });
        });
      })
    );
...
self.addEventListener('install', function(event) {
  // "use strict";
  consoleWrapper(`[Service Worker] Event: install`);
  try {
    self.skipWaiting();
    event.waitUntil(
      caches.open(fullCacheName).then(function(cache) {
...
self.addEventListener('activate', function(event) {
  // "use strict";
  consoleWrapper(`[Service Worker] Event: activate`);
  try {
...
self.addEventListener('fetch', function(event) {
  // "use strict";
  try {
    consoleWrapper(`[Service Worker] Fetch request: ${event.request.url}`);
    event.respondWith(
      caches.match(event.request).then(function(request) {
        if (request) {
          ...
          consoleWrapper(`[Service Worker] Resource not cached, fetching: ${event.request.url}`);
...
self.addEventListener('updatefound', (event) => {
  consoleWrapper('[Service Worker] New service worker version available');
