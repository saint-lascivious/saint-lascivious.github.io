        ...
        return Promise.all(
          allURLS.map(function(url) {
            return fetch(url).then(function(response) {
              ...
              consoleWrapper(`[Service Worker] Cached ${url}`);
            });
          })
        );
      })
...
self.addEventListener('fetch', function(event) {
  ...
  try {
    ...
    if (!requestUrl.origin.startsWith(baseDomain)) {
    ...
    event.respondWith(
      caches.match(event.request).then(function(response) {
        ...
        consoleWrapper(`[Service Worker] Fetching asset from network: ${event.request.url}`);
