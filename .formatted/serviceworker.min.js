...
const cacheName = 'sainternet';
const cacheVersion = '_v32';
...
/**
 * Yo dawg, I heard you like arrays, so we hooked you up with an array of
 * arrays.
 */
const allURLS = [
  ...cssURLS,
...
/**
 * Periodic updater, adds all the URLs in `allURLS` array to the`fullCacheName`
 * cache.
 *
 * @function
 * @name periodicCacheFetch
 */
function periodicCacheFetch() {
  // "use strict";
  try {
    event.waitUntil(
      caches.open(fullCacheName).then(function(cache) {
        consoleWrapper(`[Service Worker] Updating cache: ${fullCacheName}`);
        return cache.addAll(allURLS);
      }).then(function() {
        consoleWrapper(`[Service Worker] Updated cache: ${fullCacheName}`);
      })
    );
  } catch (error) {
    consoleWrapper(`[Service Worker] Error updating cache: ${error}`, 'error');
  }
};

/**
 * Logs a message to the console, with optional date and time information,
 ...
 *
 * Calls `periodicCacheFetch` every 12 hours using `setInterval`, after
...
self.addEventListener('install', function(event) {
  // "use strict";
  consoleWrapper(`[Service Worker] Event: install ${cacheName}`);
  try {
    self.skipWaiting();
    event.waitUntil(
      caches.open(fullCacheName).then(function(cache) {
        consoleWrapper(`[Service Worker] Installing cache: ${fullCacheName}`);
        return cache.addAll(allURLS);
      }).then(function() {
        setInterval(periodicCacheFetch, 12 * 60 * 60 * 1000);
      })
    );
  } catch (error) {
...
self.addEventListener('activate', function(event) {
  // "use strict";
  consoleWrapper(`[Service Worker] Event: activate ${cacheName}`);
  try {
    ...
    event.waitUntil(
      caches.keys().then(function(keyList) {
        const cacheWhitelist = keyList.filter(function(key) {
          return key.indexOf(cacheName);
        });
        consoleWrapper(`[Service Worker] Whitelisting current cache: ${fullCacheName}`);
        cacheWhitelist.push(fullCacheName);
        return Promise.all(keyList.map(function(key, i) {
          if (cacheWhitelist.indexOf(key) === -1) {
            consoleWrapper(`[Service Worker] Deleting orphaned cache: ${keyList[i]}`);
 ...
 *
 * Also intercepts requests to Google Fonts assets and caches the responses.
 *
    ...
    if (url.origin === 'https://fonts.googleapis.com' || url.origin === 'https://fonts.gstatic.com') {
      event.respondWith(
        caches.match(event.request).then(function(cachedResponse) {
          if (cachedResponse) {
            consoleWrapper(`[Service Worker] Responding from cache: ${event.request.url}`);
            return cachedResponse;
          } else {
            consoleWrapper(`[Service Worker] Responding from network: ${event.request.url}`);
            return fetch(event.request).then(function(networkResponse) {
              if (networkResponse.ok) {
                const cache = caches.open('google-fonts-cache');
                cache.put(event.request, networkResponse.clone());
              }
              return networkResponse;
            }).catch(function(error) {
              consoleWrapper(`[Service Worker] Network request failed: ${error}`, 'error');
 ...
 * registration object.
 *
 * Logs the service worker update life cycle.
 *
...
self.addEventListener('updatefound', (event) => {
  ...
  newServiceWorker.addEventListener('statechange', () => {
    switch (newServiceWorker.state) {
      case 'installing':
        consoleWrapper('[Service Worker] New service worker version installing');
        break;
      case 'installed':
        consoleWrapper('[Service Worker] New service worker version installed');
        break;
      case 'activating':
        consoleWrapper('[Service Worker] New service worker version activating');
        break;
      case 'activated':
        consoleWrapper('[Service Worker] New service worker version activated');
        break;
      case 'redundant':
        consoleWrapper('[Service Worker] New service worker version redundant');
        break;
    }
  });
});
...
/**
 * Listens for messages sent from the client and updates the service
 * worker's debug flag accordingly. Also responds with a "pong" message
 * for "ping" messages.
 *
 * @function
...
self.addEventListener('message', (event) => {
  if (event.data.type === 'debug') {
    ...
    consoleWrapper(`[Service Worker] DEBUG flag set to ${debugValue}`);
  } else if (event.data === 'ping') {
    consoleWrapper('[Service Worker] Pong');
  }
});
