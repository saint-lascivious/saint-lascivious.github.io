...
const cacheVersion = '_v15';
const fullCacheName = `${cacheName}${cacheVersion}`;

/**
 * Domain arrays (to be cached) are listed here.
 */
const appURLS = [
...
/**
 * Calls `periodicCacheFetch` every 12 hours using `setInterval`.
 *
...
/**
 * Event listener for the `sainternet-broadcasts` channel using the Broadcast
 * Channels API.
 *
 * Used to listen for changes to the `DEBUG` flag value.
 */
if ('BroadcastChannel' in window) {
  const channel = new BroadcastChannel('sainternet-broadcasts');
  channel.addEventListener('message', function(event) {
    consoleWrapper('[Service Worker] Received message from broadcast channel');
    const debugFlag = event.data.debug;
    consoleWrapper(`[Service Worker] DEBUG flag set to ${debugFlag}`);
  });
} else {
  consoleWrapper('[Service Worker] Broadcast Channel API not supported');
}
 ...
 * @param {string} [logType='log'] - The type of logging method to use (e.g.
 * 'log', 'warn', 'error', etc.).
 * @return {void}
 */
function consoleWrapper(message, logType = 'log') {
  // "use strict";
  if (debugFlag === 'true') {
