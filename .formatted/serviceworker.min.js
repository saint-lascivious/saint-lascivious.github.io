...
/**
 * Adds all URLs in `allURLS` to the `fullCacheName` cache.
 *
 * @function
...
function handlePeriodicUpdate() {
  ...
  caches.open(fullCacheName).then(function(cache) {
    logDebugMessage(`[Service Worker] Updating cache: ${fullCacheName}`);
...
self.addEventListener('install', function(event) {
  ...
  try {
    ...
    event.waitUntil(
      caches.open(fullCacheName).then(function(cache) {
        logDebugMessage(`[Service Worker] Installing cache: ${fullCacheName}`);
        return cache.addAll(allURLS);
      })
    );
  } catch (error) {
  ...
  try {
    ...
    event.waitUntil(
      caches.keys().then(function(keyList) {
        ...
        logDebugMessage(`[Service Worker] Whitelisting current cache: ${fullCacheName}`);
