...
function consoleWrapper(message, logType = 'log', includeDateTime = false) {
  if (self.CONSOLE_LOGGING && includeDateTime) {
...
/**
 * Custom sleep implementation, since apparently 'sleep()' just plain
 * isn't a thing in JavaScript.
 *
 * Wait for a duration. If a duration is not specified, wait for a random
 * duration between 100 and 2500 (in multiples of 50). If a duration is
 * specified, check it is a positive integer and no larger than 5000.
 *
...
function sleepFor(sleepDuration) {
  if (Number.isInteger(sleepDuration) || sleepDuration > 0) {
    sleepDuration = Math.floor(sleepDuration);
    sleepDuration = Math.min(sleepDuration, 5000);
    if (!sleepDuration) {
    ...
    if (!Number.isInteger(sleepDuration)) {
      consoleWrapper(`[Service Worker] Sleep duration is not an integer`, 'error');
    } else {
      consoleWrapper(`[Service Worker] Sleep duration is not a positive integer`, 'error');
    };
  };
}

/**
 * Event listener for the service worker 'activate' event.
 *
 * The `activate` event is triggered when the service worker becomes active
 * either due to a new installation or an update.
 *
 * This event deletes any caches that have a name that does not contain
 * `shortName` or `cacheName`, where `shortName` is a global constant defined
 * elsewhere in the service worker code. It also claims any clients that are
 * available.
 *
    ...
    event.waitUntil(
      caches.keys().then(function(keyList) {
        ...
        if (registration.updateIntervalId) {
          clearInterval(registration.updateIntervalId);
          delete registration.updateIntervalId;
          consoleWrapper(`[Service Worker] Cleared periodic update interval`);
        }
      })
    );
  } catch (error) {
...
/**
 * Event listener for the service worker 'install' event.
 *
 * The `install` event is triggered when the browser installs the service
 * worker for the first time or when the service worker code has been updated.
 *
 * This event fetches and caches an array of URLs named `cacheOnInstall`
 * using the given cache name `cacheName`.
 *
 * It then sets a periodic background update event every 6 hours to fetch and
 * cache an array of URLs named `cacheCritical`. The update event is set using
 * the `setInterval` function and the `fetchCacheUrlArray` function is used to
 * fetch and cache the URLs.
 *
  ...
  try {
    event.waitUntil(
      fetchCacheUrlArray(cacheName, cacheOnInstall).then(function() {
        consoleWrapper(`[Service Worker] Finished caching`);
        const updateInterval = 6 * 60 * 60 * 1000;
        const registration = self.registration;
        const intervalId = setInterval(function() {
          consoleWrapper(`[Service Worker] Updating critical cache`);
          fetchCacheUrlArray(cacheName, cacheCritical).catch(function(error) {
            consoleWrapper(`[Service Worker] Error during cache update: ${error}`, 'error');
          });
        }, updateInterval);
        registration.updateIntervalId = intervalId;
 ...
 *
 * Current events are 'flag', 'info', and 'ping'.
 *
 * @function
...
self.addEventListener('message', function(event) {
  ...
  } else if (event.data.type === 'flag') {
    const {flag, value} = event.data;
    self[flag] = value;
    consoleWrapperSimple(`[Service Worker] ${flag}: ${value}`);
  } else if (event.data.type === 'stored_flag') {
    const {flagFromStorage, valueFromStorage} = event.data;
    self[flagFromStorage] = valueFromStorage;
  }
});
