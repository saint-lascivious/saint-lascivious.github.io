  ...
  try {
    ...
    event.waitUntil(
      caches.open(cacheName).then(function(cache) {
        consoleWrapper(`[Service Worker] Installing cache: ${cacheName}`);
        return Promise.all(
          cacheCritical.map(function(url) {
  ...
  try {
    ...
    event.waitUntil(
      caches.keys().then(function(keyList) {
        ...
        consoleWrapper(`[Service Worker] Whitelisting current cache: ${cacheName}`);
...
self.addEventListener('fetch', function(event) {
  ...
  try {
    if (event.request.method !== 'GET') {
      consoleWrapper(`[Service Worker] Method not eligible for fetch: ${event.request.method}`);
      return;
    }
