...
const VERSION_DAY = '21';
const VERSION_HOUR = '20';
const VERSION_MIN = '45';

const VERSION_DATE = VERSION_YEAR + VERSION_MONTH + VERSION_DAY;
const VERSION_TIME = VERSION_HOUR + VERSION_MIN;

const PWA_CACHE_VERSION = VERSION_DATE + VERSION_TIME;

const PWA_PREFIX = 'sainternet_';
const PWA_VERSION_PREFIX = 'version_';

const PWA_CACHE_PREFIX = PWA_PREFIX + PWA_VERSION_PREFIX;
...
self.addEventListener('fetch', function (event) {
  // "use strict";
  event.respondWith(
    cache.open(PWA_CACHE_NAME)
      .then(function (cache) {
        return cache.match(event.request, cacheOptions)
...
self.addEventListener('install', function (event) {
  ...
  event.waitUntil(
    ...
    .catch(function (error) {
      console.error("[Service Worker] Cache Initiation Failed:", error);
    })
  );
});
      ...
      .then(function (cacheNames) {
        return Promise.all(
          cacheNames.filter(function (cacheName) {
            return cacheName.startsWith(PWA_CACHE_PREFIX) && cacheName !== PWA_CACHE_NAME;
