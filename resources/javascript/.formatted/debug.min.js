...
/**
 * Sends a "cacheAssets" message to the service worker.
 *
 * @name getServiceWorkerInfo
 * @function
 * @return {void}
 */
function cacheServiceWorkerAssets() {
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.ready
      .then((registration) => {
        consoleWrapperSimple('[Debug] Sending service worker cacheAssets message');
        registration.active.postMessage({ type: 'cacheAssets', urls: cacheAdditional });
      }
    );
  }
}
...
function pingServiceWorker() {
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.ready
      .then((registration) => {
        consoleWrapperSimple('[Debug] Ping');
