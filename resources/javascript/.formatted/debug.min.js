...
/**
 * Toggles the true/false state of a flag in local storage.
 *
 * @function
 * @name toggleFlag
 * @param {string} flagName - The name of the flag to toggle.
 * @return {boolean} - The new value of the flag after toggling.
 */
function toggleFlag(flagName) {
  const flagValue = sessionStorage.getItem(flagName);
  let newValue = false;
  if (flagValue === 'true') {
    consoleWrapper(`[Debug] ${flagName}: false`);
    newValue = false;
    sessionStorage.setItem(flagName, 'false');
  } else {
    newValue = true;
    sessionStorage.setItem(flagName, 'true');
    consoleWrapper(`[Debug] ${flagName}: true`);
  }
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.ready.then((registration) => {
      registration.active.postMessage({type: 'flag', name: flagName, value: newValue});
    });
  }
  return newValue;
}
