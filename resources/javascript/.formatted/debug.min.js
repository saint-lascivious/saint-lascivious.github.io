...
/**
 * Deletes all caches with names that start with the specified prefix.
 *
 * The cache name prefix is defined inside the function as `pwaPrefix`.
 *
 * @function
 * @name deleteServiceWorkerCaches
 * @return {Promise} - A Promise that resolves when all matching caches are
 * deleted.
 */
function deleteServiceWorkerCaches() {
  // "use strict";
  const pwaPrefix = 'sainternet';
  caches.keys().then((keys) => {
    const cacheKeys = keys.filter((key) => key.startsWith(pwaPrefix));
    ...
    const deletedCaches = results.filter((result) => result === true);
    if (deletedCaches.length === 0) {
      logDebugMessage(`[Debug] No matching caches found`);
    } else {
      logDebugMessage(`[Debug] Deleted ${deletedCaches.length} matching caches`);
    }
  }).catch((error) => {
    logDebugMessage(`[Debug] Error deleting caches: ${error}`, 'error');
  });
}

/**
 * Toggles the debug flag value.
 *
 * @function
