...
/**
 * Toggles the CONSOLE_LOGGING flag in session storage.
 *
 * @function
...
function toggleConsoleLogging() {
  ...
  if (consoleFlag === 'true') {
    consoleValue = false;
    consoleWrapper(`[Debug] CONSOLE_LOGGING: ${consoleValue}`);
    localStorage.setItem('CONSOLE_LOGGING', `${consoleValue}`);
  } else {
    consoleValue = true;
    localStorage.setItem('CONSOLE_LOGGING', `${consoleValue}`);
    consoleWrapper(`[Debug] CONSOLE_LOGGING: ${consoleValue}`);
...
/**
 * Broadcasts the current CONSOLE_LOGGING flag value to a registered
 * service worker on debug.js load if found to be true.
 *
 * @function
 * @name broadcastConsoleLoggingState
 * @return {void}
 */
function broadcastConsoleLoggingState() {
  if ('serviceWorker' in navigator) {
    const consoleFlag = localStorage.getItem('CONSOLE_LOGGING');
    let consoleValue = false;
    if (consoleFlag === 'true') {
