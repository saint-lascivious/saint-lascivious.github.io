...
/**
 * Toggles the DEBUG flag in local storage and broadcasts it using the
 * Broadcast Channels API on the 'sainternet-broadcasts' channel.
 *
 * @function
...
function toggleDebugFlag() {
  ...
  if (debugFlag === 'true') {
    consoleWrapper('[Debug] Toggle DEBUG: false');
    debugValue = false;
    localStorage.setItem('DEBUG', 'false');
  } else {
    debugValue = true;
    localStorage.setItem('DEBUG', 'true');
    consoleWrapper('[Debug] Toggle DEBUG: true');
  }
  const channel = new BroadcastChannel('sainternet-broadcasts');
  channel.postMessage({debug: debugValue});
  channel.close();
  return debugValue;
}
